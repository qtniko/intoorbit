...
# Switching to JS (p5.js) for web integration (starting v0.0.3)
...
class Body():
    ...
    def drawTrail(self, surface):
        if len(body.trail) > trailLength:
            body.trail = body.trail[1:]
        
        if len(body.trail) > 1:
            for idx, tpl in enumerate(body.trail[1:]):
                pos, color = tpl
                # print(int(trailStartingWidth*((idx+1)/trailDecrease)))
                pygame.draw.line(surface, color, body.trail[idx][0], pos, int(trailStartingWidth*(idx/trailDecrease)))
...
trailLength = 300
trailStartingWidth = orbitRadius
trailDecrease = 600
...
while 1:
    ...
    for body in bodies:
        ...
        try:
            body.trail.append((body.pos+0, clr))
            body.drawTrail(screen)
